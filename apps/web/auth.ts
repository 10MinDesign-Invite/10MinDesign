...
//             }
//               const res = await axios.post(`${process.env.NEXT_PUBLIC_Backend_URL}/add/user`,{email,id,name,image});
//               if(res.status === 200){
//                 return true
//               }else{
//                 return false
...
const config: NextAuthConfig = {
  ...
  callbacks:{
    signIn:async ({user,account,email,profile})=>{
      ...
      async jwt({token}){
        const existing_user = await axios.post(`${process.env.NEXT_PUBLIC_Backend_URL}/verify/user`,{email:token.email,customeData:"select only id and role"});
